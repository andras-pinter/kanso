// @ts-nocheck
// Vendored from @blocksuite/integration-test/src/editors/editor-container.ts
// (BlockSuite v0.22.4, MIT-licensed, © toeverything). Copied so we can drop
// the test-scoped `@blocksuite/integration-test` package as a production dep
// while still owning the `<affine-editor-container>` custom element. Only
// change vs upstream: class renamed TestAffineEditorContainer -> AffineEditorContainer.
//
// Registration is performed once by ./internal.ts via customElements.define.
//
// See ui/src/blocksuite-stub.ts for why the `@blocksuite/*` imports are typed
// as `any` under tsc.
