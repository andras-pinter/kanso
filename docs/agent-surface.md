...
| Create column                  | `POST /boards/:board_id/columns`                | `column_create`                      | `column_create`                            | `column_create`                            | `201 ColumnDto`                                              | Body: `{name, position?}`. |
| Update column                  | `PATCH /columns/:id`                            | `column_update`                      | `column_update`                            | `column_update`                            | `ColumnDto`                                                  | Patch: `name`, `position`. |
...
| Create card                    | `POST /columns/:column_id/cards`                | `card_create`; legacy `create_card`  | `kanso_add`; `card_create`                 | `kanso_add`; `card_create`                 | `201 CardDto`                                                | `kanso_add` optionally patches body_text after create; typed `card_create` is title-only. |
| Update card                    | `PATCH /cards/:id`                              | `card_update`                        | `card_update`                              | `card_update`                              | `CardDto`                                                    | Patch: `title`, `body_text` (nullable), `due_at` (nullable). |
...
| Get card body (BlockSuite)     | `GET /cards/:id/body`                           | `card_body_get`                      | `card_body_get`                            | `card_body_get`                            | `CardBodyDto {body_blocksuite_b64, body_text, updated_at}`   | 8 MiB PUT cap on paired setter. |
| Set card body                  | `PUT /cards/:id/body`                           | `card_body_set`                      | `card_body_set`                            | `card_body_set`                            | `CardDto`                                                    | At least one of `body_text`/`body_blocksuite_b64` required; a missing field clears that column. Text-only writes let agents write plaintext without synthesizing a BlockSuite blob (UI seeds a fresh editor from the text on next open). |
...
| `PUT /cards/:id/body`                   | `CardDto` (at least one of `body_text`/`body_blocksuite_b64` required; missing field clears that column) |
